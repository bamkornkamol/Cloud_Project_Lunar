<template>
  <section id="app" style="font-family: 'Prompt', sans-serif">
    ...
    <div class="flex flex-row mt-14 mx-20">
      <div class="basis-5/6">
        <h1 class="text-6xl font-medium tracking-wide mb-5">
          {{ dormitory[0].name }}
        </h1>
        <p class="text-xl">
          {{ dormitory[0].address }} {{ dormitory[0].parish }}
          {{ dormitory[0].district }} {{ dormitory[0].province }}
          {{ dormitory[0].post }}
        </p>
    ...
    <div class="w-full flex justify-center items-center mt-5 mb-20">
      <div class="w-9/12 border-2 border-[#2E4E73] p-5 rounded-2xl">
        ...
        <p class="mb-2">
          จำนวนห้องว่าง :
          {{ parseInt(dormitory[0].room) - parseInt(renter.length) }} ห้อง
        </p>
        <p class="mb-2">ห้องที่มีผู้เช่า :</p>
        <div class="grid grid-cols-12 gap-2">
          <div v-for="val in renter" :key="val">
            <div
              class="mb-2 bg-[#2E4E73] text-center text-white rounded-xl p-1 text-sm"
            >
              {{ val.num_room }}
            </div>
          </div>
...
export default {
  ...
  created() {
    axios
      .get(
        "http://47.129.60.17:3000/Dormitory/" +
          this.$route.params.userId +
          "/" +
          this.$route.params.dorId
      )
      .then((response) => {
